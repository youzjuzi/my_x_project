...
# M/N 分类（双侧硬边界 + 深度锁死）
# =========================
def classify_mn_robust(hand, fist_family, th_gap=0.55, th_e=0.45, th_seg=0.18, th_z=0.10):
    if not fist_family:
        return "not_fist", {}
    ...
    t_middle = get_proj_t(point_xy(hand[9]))  # 新增：中指根部边界 (N的左侧防线)
    t_ring = get_proj_t(point_xy(hand[13]))   # 无名指根部边界 (N的右侧防线)
    ...
    if t_thumb > t_ring - 0.02:
        ...
        # 没越过无名指，但越过了中指（-0.05是给大拇指肉厚度留的容差），是 N
        best_name = "N"
    else:
        # 连中指都没越过！说明它放在了食指和中指之间（T区），或者更外面。直接枪毙！
        base_info["best"] = "T_zone_rejected"
        return "not_mn", base_info
    ...
    raw_best_d = d_m_raw if best_name == "M" else d_n_raw
    ...
    cls = best_name if inserted else "not_mn"
...
def to_mn_or_yolo(cls):
    return cls if cls in ["M", "N"] else "yolo"
    ...
    while True:
        ...
        if result.hand_landmarks:
            ...
            if tracking_active:
                ...
                # 调用更新后的分类函数
                raw_cls_base, info = classify_mn_robust(
                    hand,
                    True,
                ...
                candidate_raw = to_mn_or_yolo(raw_cls_base)

                if candidate_raw in ["M", "N"]:
            ...
            if info:
                # 只绘制 N 和 M 的 gap 辅助点
                for name, gap in [("N", info.get("gap_n")), ("M", info.get("gap_m"))]:
                    if gap is None:
                        continue
                    gx = int(gap[0] * w)
                    gy = int(gap[1] * h)
            ...
            cls_color = (0, 0, 255) if stable_cls in ["M", "N"] else (200, 200, 200)
            ...
            if info:
                d_n = info.get("dN_raw", -1)
                d_m = info.get("dM_raw", -1)
                t_palm = info.get("t_palm", -1)
                d_e = info.get("dE", -1)
                best = info.get("best")
                ...
                # 更新显示为新的特征指标
                cv2.putText(frame, f"dN={d_n:.3f} dM={d_m:.3f} t_palm={t_palm:.3f} dE={d_e:.3f}", (20, 190), cv2.FONT_HERSHEY_SIMPLEX, 0.60, (255, 255, 255), 2)
                cv2.putText(frame, f"best={best}", (20, 220), cv2.FONT_HERSHEY_SIMPLEX, 0.60, (255, 255, 255), 2)
                ...
                cv2.putText(
                    frame,
                    f"TH_GAP={TH_GAP:.2f}/{TH_GAP_KEEP:.2f} TH_SEG={TH_SEG:.2f}/{TH_SEG_KEEP:.2f} TH_Z={TH_Z:.2f}/{TH_Z_KEEP:.2f}",
                    (20, 370),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    0.50,
                    (255, 255, 255),
